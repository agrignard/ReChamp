...
global {   
	bool small <- false parameter: true;
	//file shape_file_roads  <- small ? file("../includes/roads_7200_with_pm_small_small-Nettoye.shp") : file("../includes/roads_7200_with_pm_small-Nettoye.shp") ;
	//file shape_file_nodes  <-  small ?file("../includes/nodes_7200_with_pm_small_small_Nettoye.shp") : file("../includes/nodes_7200_with_pm_small_Nettoye.shp");
	file shape_file_roads  <- small ? file("../includes/roads_7200_with_pm_small_small-Nettoye.shp") : file("../includes/roads_7200_pm_ok_v2.shp") ;
	file shape_file_nodes  <-  small ?file("../includes/nodes_7200_with_pm_small_small_Nettoye.shp") : file("../includes/nodes_7200_pm_ok_v1.shp");
	file shape_urgence  <-  small ?file("../includes/Evacuation_L93_small.shp") : file ("../includes/evac_big-v2.shp") ;
	...
	reflex save_people_location when: time = 2.0 or every(600.0) {
		save people type:"shp" to:"../models/shp_people/shape_people_" + time + ".shp" crs: "EPSG:2154" ;
		//save embouteillage type:"shp" to:"../models/shp_traffic-jam/shape_t-j_" + time + ".shp" crs: "EPSG:2154" ;
	}
	
/* 	reflex save_usage_route when: every(10) {
		ask road {
			if  (nb_personnes > 0) {
			...
			write temps_moy;		
				...
				//temps_moy <- 0 ;
			//}
		save road type:"shp" to:"../models/shp_route/shape_usage_route_" + time + ".shp" crs: "EPSG:2154" with:[temps_moy::"Tps_Moy"] ;		
		}
	} */
	...
	reflex save_usage_route when: every(10) {
        ...
        save road type:"shp" to:("../models/shp_route/shape_usage_route_" + cycle + ".shp") crs: "EPSG:2154" with:[temps_moy::"Tps_Moy", nb_personnes::"people"] ;				
	...
	init {  
		...
		create road from: shape_file_roads with:[nb_agents::int(read("NB_CARS")), name::string(read("name")),highway::string(get("highway")),junction::string(read("junction")),lanes::int(read("lanes")), maxspeed::float(read("maxspeed")) #h/#km, oneway::string(read("oneway")), lanes_forward ::int (get( "lanesforwa")), lanes_backward :: int (get("lanesbackw")), priority::float((get("priority"))), target_evacuation ::int(get("evacuation"))] {
			...
			min_traffic_jam_people_destroy <- int(min([capacite_max / 2.0, min_embouteillage_people_destruction]));
			min_traffic_jam_people_creation <- int( min([capacite_max/2.0, min_embouteillage_people_creation]));
		...
		//save priority and target_evacuation of roads in the shapefile roads_7200_pm_ok_v2
		/*ask road {maxspeed <- maxspeed #km/#h;}
		save road to: ("../includes/roads_7200_pm_ok_v2.shp") type: "shp"
	...
	action compute_shortest_paths {
		if save_shortest_paths {
			matrix ssp <- all_pairs_shortest_path(dri);
		...
		loop pt over: pts {
			loop i from: 0 to: a_road.lanes -1 {
				create people  { 
					...
					proba_lane_change_up <- flip(proportion_fous) ? 1.0 : 0.8 ;// 0.5 + (rnd(500) / 500);
					proba_lane_change_down <- flip(proportion_fous) ? 0.5 : 1.0;//0.7+ (rnd(300) / 500);
					location <- pt;
					current_lane <- i;
					init_rd <- a_road;
					security_distance_coeff <- flip(proportion_fous) ? 0.5 : 2 * (1.5 - rnd(1.0)); //  
					proba_respect_priorities <- flip(proportion_fous) ? 0.5 : 1.0;
					proba_respect_stops <- flip(proportion_fous) ? [0.5] : [1.0]; // POURQUOI CROCHETS ? 
					proba_block_node <- flip(proportion_fous) ? 0.1 : 0.0;
					proba_use_linked_road <- 0.0;
					max_speed <- flip(proportion_fous) ? 150 : 150 #km/#h;
					max_acceleration <- 1000.0;//(12 + rnd(500) / 100) #km/#h;
					speed_coeff <- flip(proportion_fous) ? 2.0 : 1.2 - (rnd(400) / 1000);
...
species danger { 
	aspect base {
		draw shape border: #yellow color: #yellow ;
	}
}
...
species people skills: [advanced_driving] schedules: [] { 
	...
	aspect base { 
		draw triangle(3) color: color_behavior border: #black rotate:heading + 90;	
	} 
	...
	float external_factor_impact(road new_road,float remaining_time) {
		...
		if (proba_avoid_traffic_jam > 0 and flip(proba_avoid_traffic_jam)) {
			...
			roads_traffic_jam <- remove_duplicates(roads_traffic_jam + (current_node_tmp.neighbours_tj));// where (each.embout_route != nil and each.embout_route.real));// and not (each in roads_traffic_jam)));
...
experiment traffic_simulation_sc_exceptionnel type: gui {
	...
	/* 	display carte_embouteillage{
		 ...
		 display city_display type: opengl refresh: every(1){ // retirer opengl
