...
global {
	...
	graph driving_road_network_pro;
	graph driving_road_network_exi;
...
//	map<road,float> proba_use_road;
  ...
  //	list<point,geometry> queue_per_loc;
	...
	init {
		...
		driving_road_network_exi <- (as_driving_graph(road, intersection)) ;
		vertices <- list<intersection>(driving_road_network_exi.vertices);
...
//		proba_use_road <- road as_map (each::each.proba_use);
		...
		driving_road_network_exi <- driving_road_network_exi with_weights general_speed_map; 
		
		list<road> pro_roads <- road where (each.pro_lanes > 0);
		map general_speed_map_pro <- pro_roads as_map (each::((each.hot_spot ? 1 : 10) *(each.shape.perimeter / each.maxspeed) / (1+each.lanes)));
		
		driving_road_network_pro <- (as_driving_graph(pro_roads, intersection - tf_can_be_desactivated)) with_weights general_speed_map_pro;
	}
	...
	action create_cars(int nb) {
		create car number:nb{
			...
			current_intersection <- one_of(intersection - output_intersections -tf_can_be_desactivated);
	...
	action updateSimuState {
		...
		if (currentSimuState = 1){	
			...
			list<road> blocked_roads <- road where (each.pro_lanes = 0);
			list<car> car_to_delete <- car where ((each.target_intersection in tf_can_be_desactivated) or each.use_blocked_road()) ;
			//do create_cars(length(car_to_delete));
			ask car_to_delete{
				write name+ " + objectif interdit: " + (target_intersection in tf_can_be_desactivated) + " route interdit: " + use_blocked_road();
				if (current_road != nil) {
...
species car skills:[advanced_driving]{	
	...
	reflex leave when: final_target = nil  {
		...
		current_lane <- 0;
		current_path <- compute_path(graph: currentSimuState = 0 ? driving_road_network_exi : driving_road_network_pro, target: target_intersection);
	}
...
experiment ReChamp type: gui autorun:true{
	...
	output {
		display champ type:java2D background:#black draw_env:false  rotate:angle toolbar:false autosave:false synchronized:true
