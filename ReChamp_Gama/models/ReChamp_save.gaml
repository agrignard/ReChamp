...
global {
	...
	bool use_sp_files <- false;
	bool save_shortest_paths <- false;
	
	file file_ssp_future;
	file file_ssp_present;
...
//		proba_use_road <- road as_map (each::each.proba_use);
		...
		people_graph <- as_edge_graph(road) with_optimizer_type "FloydWarshall";
	...
	action fill_ssp_matrix {
		if (file_exists("../includes/path/present_sp.csv")) {
			file_ssp_present <- csv_file("../includes/path/present_sp.csv",";");
		}
		if (file_exists("../includes/path/future_sp.csv")) {
			file_ssp_future <- csv_file("../includes/path/future_sp.csv",";");
		}
		/*if (file_exists("../includes/path/bike.csv")) {
			bike_graph <- bike_graph load_shortest_paths matrix(file("../includes/path/bike.csv"));
		}
		if (file_exists("../includes/path/pedestrian.csv")) {
			people_graph <- people_graph load_shortest_paths matrix(file("../includes/path/pedestrian.csv"));
		}*/
	}
	...
	action compute_shortest_paths {
		if save_shortest_paths {
			driving_road_network <- driving_road_network with_optimizer_type "FloydWarshall";
			matrix ssp <- all_pairs_shortest_path(driving_road_network);
			write "Matrix of all shortest present: " + length(ssp);
			save ssp type:"text" to:"../includes/path/present_sp.csv";
			...
			write "Matrix of all shortest future: " + length(ssp);
			save ssp type:"text" to:"../includes/path/future_sp.csv";
			
		//	save all_pairs_shortest_path(bike_graph with_optimizer_type "FloydWarshall")  type:"text" to:"../includes/path/bike.csv";
		//	save all_pairs_shortest_path(people_graph with_optimizer_type "FloydWarshall") type:"text" to:"../includes/path/pedestrian.csv";
		} 
	}
...
species car skills:[advanced_driving]{	
	...
	action compute_path_opt {
		file file_ssp_speed <- currentSimuState_str = "present" ? file_ssp_present : file_ssp_future;
		if (file_ssp_speed != nil) {
			list<intersection> nodes <- world.nodes_for_path(current_intersection,target_intersection,file_ssp_speed);
			if (length(nodes) > 1) {
				current_path <- path_from_nodes(graph: driving_road_network, nodes: nodes);
			} else {
				current_path <- nil;
				final_target <- nil;
			}
