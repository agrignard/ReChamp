...
global{
	...
	shape_file table <- shape_file("../../includes/GIS/TableBounds.shp");

	//OSM file to load
	file<geometry> osmfile <-  file<geometry>(osm_file("../../includes/GIS/champs2.pbf", filtering))  ;
	
	geometry shape <- envelope(table);
	graph the_graph; 
	...
	init {
		...
		save road type:"shp" to:"../includes/roads_OSM.shp" attributes:["lanes"::lanes,"maxspeed"::maxspeed,"oneway"::oneway] ;
		save intersection type:"shp" to:"../includes/nodes_OSM.shp" attributes:[type::"type", crossing::"crossing"] ;
